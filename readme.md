...
The analysis is a [`targets`](https://books.ropensci.org/targets/) pipeline. Nothing needs configuring: every raw input is fetched programmatically by `R/download.R`, so there are no machine-specific paths to edit and no data to place by hand.
...
| Network | **~4.7 GB** on a first run | Dominated by the GBIF archive |
...
This downloads every input, cleans 24.4 million occurrence records, builds the analysis grid, fits the models, and writes all figures and tables to `outputs/`.
...
*Everything needed to run the analysis is above. The rest of this document covers what the pipeline produces, how it is organised, how its inputs are pinned, and where the time goes.*
...
| `rarefaction_retention.csv` | Rarefied richness as a percentage of raw, by taxon grouping and climate quartile |
...
96 targets. `targets::tar_visnetwork()` draws the full interactive graph; the overall shape is:
...
Taxonomic alignment contacts the Australian Plant Census on every rebuild rather than only the first, so a fully offline rebuild is possible only once that step is current.
...
Measured on an Apple M4 (10 cores, 32 GB RAM, SSD) with a sustained download rate of about 95 MB/min. Regenerate with:
...
| Render figures and tables | ~12 s | 17 figures, 8 tables |
| **Total** | **~30 min** | |
